...
def generate_graphs(distance_file_path, charging_file_path, consumption_file_path, folder, output_folder):
    ...
    pertime_avg = df.groupby('hour')['vehicles_at_depot'].mean()
    
    # Plot pertime average of vehicles at depot
    plt.figure(figsize=(10, 6))
    plt.bar(pertime_avg.index, pertime_avg, label='pertime Average of Vehicles at Depot', color="#04643F")
    ...
    #plt.title('pertime Average of Vehicles at Depot', fontsize=18)
    plt.grid(True, axis='y')
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, f'pertime_avg_vehicles_at_depot_{folder}.jpeg'), format='jpeg', dpi=600)
    plt.show()
    ...
    pertime_avg_consumption = energy_consumption.groupby('hour')['Total_Consumption_kWh'].mean()
    
    # Plot pertime energy consumption
    plt.figure(figsize=(10, 6))
    ...
    #plt.title('pertime Average Energy Consumption', fontsize=18)
    ...
    plt.gca().spines['right'].set_visible(False)
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, f'pertime_avg_energy_consumption_{folder}.jpeg'), format='jpeg', dpi=600)
    plt.show()
